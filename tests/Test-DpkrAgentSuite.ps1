...
try{
  ...
  & (Join-Path $SuiteRoot 'scripts\Install-DpkrAgentSuite.ps1') -SuiteRoot $SuiteRoot -UserHome $root -FrontierLoopSource $FrontierLoopSource -NativeUiSource $NativeUiSource -SkipCodexCliRegistration | Out-Null
  if($LASTEXITCODE-ne0){throw 'clean-install failed'};$tests+='clean-install'
  & (Join-Path $SuiteRoot 'scripts\Verify-DpkrAgentSuite.ps1') -SuiteRoot $SuiteRoot -UserHome $root -FrontierLoopSource $FrontierLoopSource -NativeUiSource $NativeUiSource -SkipCodexCliRegistration | Out-Null
  if($LASTEXITCODE-ne0){throw 'verify failed'};$tests+='verify-prepared-state'
  ...
  $suiteEntries=@($market.plugins|Where-Object {$_.name -in @('frontier-loop','native-ui-governance')});if($suiteEntries.Count-ne2){throw 'marketplace registration failed'};foreach($entry in $suiteEntries){$expected=if($entry.name-eq'frontier-loop'){$FrontierLoopSource}else{$NativeUiSource};$resolved=[IO.Path]::GetFullPath((Join-Path $root ([string]$entry.source.path))).TrimEnd([char[]]'\/');if(-not[string]::Equals($resolved,[IO.Path]::GetFullPath($expected).TrimEnd([char[]]'\/'),[StringComparison]::OrdinalIgnoreCase)){throw "Codex marketplace root-relative path mismatch: $($entry.name)"}};$tests+='codex-marketplace-layout'
  ...
  $failed=$false
  try{& (Join-Path $SuiteRoot 'scripts\Install-DpkrAgentSuite.ps1') -SuiteRoot $SuiteRoot -UserHome $other -FrontierLoopSource $FrontierLoopSource -NativeUiSource $NativeUiSource -SkipCodexCliRegistration | Out-Null}catch{$failed=$true}
  if(-not$failed){throw 'foreign AGENTS was not rejected'};$tests+='foreign-agents-fail-closed'
  & (Join-Path $SuiteRoot 'scripts\Install-DpkrAgentSuite.ps1') -SuiteRoot $SuiteRoot -UserHome $other -FrontierLoopSource $FrontierLoopSource -NativeUiSource $NativeUiSource -SkipCodexCliRegistration -ReplaceGlobalAgents | Out-Null
